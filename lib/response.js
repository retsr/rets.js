...
function Response(options) {
    ...
    .on('count', function(records){
        log.info('discovered count');
        $this.count = records;
    })
    .on('data', function(record){
        log.info('discovered record');
        $this.push(record);
        $this.records++;
    })
    .on('delimiter', function(delimiter){
        log.info('discovered delimiter');
        $this.delimiter = delimiter;
    })
    .on('maxrows', function(){
        log.info('discovered maxrows');
        $this.emit('maxrows', {
            count: $this.count,
