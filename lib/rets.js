...
function RETS(options) {
    ...
    if (typeof this.options === 'string') {
        this.config.url = this.options;
    } else {
        this.config = util._extend(this.config, this.options);
    }
     ...
     */
    this.config = util._extend(this.defaults, this.config);
     ...
     * @member {Session}
     */
    this.session = new Session(this.config.url, this.config);
...
RETS.prototype.login = function (options) {
    ...
    debug("login call: %o", this.session);
