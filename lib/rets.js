...
RETS.prototype.getMetadata = function (options) {
    var $this = this;
    var file = __dirname + '/../.metadata.json';
    debug("cache file: %o", file);
    debug("cache file exists?: %o", exists);
    var cache = null;
    var metadatatimestamp = new Date($this.session.settings.MetadataTimeStamp);
    debug("metadata timestamp: %o (%o)", metadatatimestamp, metadatatimestamp.getTime());
    ...
    if ( exists ) {
        ...
        debug("cache timestamp   : %o (%o)", cachetimestamp, cachetimestamp.getTime());

        var stale = (metadatatimestamp.getTime() > cachetimestamp.getTime());
