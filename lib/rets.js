...
RETS.prototype.getMetadata = function () {
    ...
    debug("Metadata Time Stamp: %o", MetadataTimeStamp);
    debug("Metadata Version: %o", MetadataVersion);
    debug("Metadata Cache File: %o", file);
    var exists = fs.existsSync(file);
    debug("Metadata Cache File already exists: %o", exists);
    if ( exists ) {
        ...
        debug("Cached Metadata Time Stamp: %o", CachedMetadataTimeStamp);
        debug("Cached Metadata Version: %o", CachedMetadataVersion);
        ...
        if (!stale) {
            ...
            debug("Metadata Cache successfully loaded from: %o", file);
            debug("%s", util.inspect(cache, {colors:true, depth: 1}));
        }
    } else {
            ...
            parser.addListener('end', function(result) {
                ...
                fs.writeFile(file, JSON.stringify(cache, null, 4), function(err) {
                    if(err) {debug(err);} else {
                        debug("Metadata Cache successfully written to: %o", file);
                    }
                });
                $this.emit('metadata', null, cache.payload);
            });
