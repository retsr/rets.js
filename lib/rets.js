...
RETS.prototype.getMetadata = function () {
    ...
    log.info({MetadataTimeStamp: MetadataTimeStamp}, "Time Stamp:");
    log.info({MetadataVersion: MetadataVersion}, "Version:");
    log.info({file: file}, "File:");
    var exists = fs.existsSync(file);
    log.info({exists: exists}, "File already exists:");
    if ( exists ) {
        ...
        log.info({CachedMetadataTimeStamp: CachedMetadataTimeStamp}, "Cache Time Stamp:");
        log.info({CachedMetadataVersion: CachedMetadataVersion}, "Cache Version:");
        var stale = (MetadataTimeStamp > CachedMetadataTimeStamp) && (MetadataVersion === CachedMetadataVersion);
        log.info({stale: stale}, "Cache is stale?:");
        if (!stale) {
